...
export default async function ListingDetailPage({ params }: Props) {
  ...
  if (!listing) {
    return (
      <div className="rounded-2xl border border-ink/10 bg-white p-6 text-sm text-ink/60 sm:rounded-3xl sm:p-8">
        Listing not found.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div className="space-y-3">
        <div className="relative h-52 w-full overflow-hidden rounded-2xl bg-forest/10 sm:h-56 sm:rounded-3xl">
          {images[0]?.url ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={images[0].url} alt={listing.title} className="h-full w-full object-cover" />
          ) : (
          ...
          <div className="grid gap-2 sm:gap-3 md:grid-cols-3">
            {images.slice(1, 4).map((image) => (
              ...
              <img
                ...
                src={image.url}
                alt={listing.title}
                className="h-24 w-full rounded-xl object-cover sm:h-28 sm:rounded-2xl"
              />
            ))}
          </div>
        ) : null}
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
        <div className="rounded-2xl border border-ink/10 bg-white p-4 sm:rounded-3xl sm:p-6">
          ...
          <p className="mt-2 text-sm leading-relaxed text-ink/60">{listing.description}</p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-ink/70">
            <span className="rounded-full bg-forest/10 px-3 py-1 text-sm sm:text-xs">
          ...
          <div className="mt-6">
            <ListingActions listingId={listing.id} landownerId={listing.owner_id} />
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {(
            ...
            <ReviewCard key={review.id} rating={review.rating} comment={review.comment} />
          ))}
          {(reviews ?? []).length === 0 ? (
